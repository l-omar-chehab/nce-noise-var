...
def data_logpdf(x, param):
    ...
    out = norm.logpdf(x, loc=0., scale=np.sqrt(param))
    return out
...
def _score_mean_modulation_integrand_nce(
    x,
    param_data,
    noise_type,
    param_noise,
    ...
    logpd = data_logpdf(x, param=param_data)
    logpn = noise_logpdf(x, param_data=param_data, noise_type=noise_type,
                         param_noise=param_noise, fisher=fisher, normalization=normalization)
    if modulation == 1:
        modulation = np.exp(2 * logpd - logpn)
    elif modulation == 2:
        modulation = np.exp(3 * logpd - 2 * logpn)
    out = _score_mean_integrand_mle(x, param_data=param_data) * modulation
    return out
...
def _score_var_modulation_integrand_nce(
    x,
    param_data,
    noise_type,
    param_noise,
    ...
    if modulation == 1:
        ...
        modulation = np.exp(3 * logpd - 2 * logpn)
    out = _score_var_integrand_mle(x, param_data=param_data) * modulation
    return out
...
@mem.cache  # cache to avoid losing computations (checkpointing)
def _score_mean_modulation_nce(
    param_data,
    noise_type,
    param_noise,
    modulation=1
    ...
    # compute score mean
    score_mean_modulated, abserr = integrator(
        _score_mean_modulation_integrand_nce,
        -np.inf, np.inf,
        args=(param_data, noise_type, param_noise, fisher, normalization, modulation),
        epsabs=tol
    )
    if verbose:
        print(f"Integration: score mean modulation (nce) computed with error {abserr}")
    return score_mean_modulated
...
def _score_var_modulation_nce(
    ...
    # compute score mean
    score_var_modulated, abserr = integrator(
        _score_var_modulation_integrand_nce,
        -np.inf, np.inf,
        args=(param_data, noise_type, param_noise, fisher, normalization, modulation),
        epsabs=tol
    )
    if verbose:
        print(f"Integration: score var modulation (nce) computed with error {abserr}")
    return score_var_modulated
...
@mem.cache  # cache to avoid losing computations (checkpointing)
def get_best_noiseprop(
    param_data=1.,
    noise_type='data',
    param_noise=0.5,
    n_samples=100,  # useless here
    estimation='singlence'  # useless here
):
    # compute fisher information
    fisher = _score_var_mle(param_data=param_data)

    # compute score mean
    a = _score_mean_modulation_nce(
        param_data=param_data,
        noise_type=noise_type,
        param_noise=param_noise,
        modulation=1)
    ...
    # compute score covar
    A = _score_var_modulation_nce(
        param_data=param_data,
        noise_type=noise_type,
        param_noise=param_noise,
        modulation=1)
    B = _score_var_modulation_nce(
        param_data=param_data,
        noise_type=noise_type,
        param_noise=param_noise,
        modulation=2)
    ...
    num = (1. / fisher)
    print("num: ", num)
    denom = (A**2 / fisher**3) - (B / fisher**2) + (A / fisher**2) - (a**2 / fisher**2)
    print("denom: ", denom)
    print("num / denom: ", num / denom)
    gamma = np.sqrt(np.abs(num / denom))
    print("gamma: ", gamma)
    noiseprop = 1. / (1. + gamma)
    print("noiseprop: ", noiseprop)

    return noiseprop


def get_mse_quadrature(n_samples=100, prop_noise=0.5, param_data=1.,
