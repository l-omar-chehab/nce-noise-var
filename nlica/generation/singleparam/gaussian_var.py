...
def get_model(n_comp, n_samples,
              seed, data_val=1.):
    """Gaussian Model where the tunable parameter is the covariance diagonal.
    """
    ...
    # param
    var = torch.tensor(data_val)  # will be made nn.Parameter later in the script

    # prior marginal
    # remove default nn.Parameter (none here) and replace it with our own
    source_mean = torch.zeros(n_comp)
    source_cov = var * torch.eye(n_comp)
    ...
    prior_marginal.cov_diag = nn.Parameter(var, requires_grad=True)
